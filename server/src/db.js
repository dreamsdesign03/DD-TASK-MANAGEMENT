...
// YYYY-MM-DD HH:mm:ss in IST (the format the frontend expects)
...
/* ─── Column maps: DB (snake_case) <-> sheet-style keys ──────────────────── */
...
/* ─── Transform helpers ───────────────────────────────────────────────────── */
// DB row (snake_case) -> sheet-style object with spaces/caps keys
...
// Sheet-style payload (camelCase or spaced keys) -> DB row (snake_case)
export function sheetToDb(obj, map) {
  if (!obj) return {}
  const out = {}
  // Build lookup from sheet key -> db col, and also accept the db col name directly
  const reverse = {}
  for (const [col, sheetKey] of Object.entries(map)) {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (col) out[col] = normalizeDbValue(value)
  }
  return out
}

// Convert legacy 'Yes'/'No'/'' booleans and booleans to DB boolean
...
// DB boolean -> legacy sheet 'Yes'/'No'
export function sheetBoolean(value) {
  return value ? 'Yes' : 'No'
}

// Build a full sheet-keyed task object, applying legacy defaults for missing fields
export function taskRowToSheet(row) {
  ...
  out['Is Active'] = out['Is Active'] === undefined ? 'Yes' : out['Is Active']
  return out
}

// Convert a task object (sheet keys / camelCase) to a DB row
export function taskPayloadToDb(payload) {
  return sheetToDb(payload, TASK_MAP)
}

// Team row -> login/user response shape the frontend expects
...
/* ─── ID generators ───────────────────────────────────────────────────────── */
function nextNumericId(rows, prefix) {
  let max = 0
  for (const r of rows) {
...
export async function nextEmployeeId() {
  ...
  const rows = (data || []).map((r) => r.employee_id)
  return 'EMP-' + String(nextNumericId(rows, 'EMP-')).padStart(3, '0')
}

export async function nextClientId() {
  const { data } = await supabase.from('clients').select('client_id')
  const rows = (data || []).map((r) => r.client_id)
  return 'C-' + String(nextNumericId(rows, 'C-')).padStart(3, '0')
}

export function nextTaskId(existing) {
  return 'T-' + String(nextNumericId(existing, 'T-')).padStart(3, '0')
}
