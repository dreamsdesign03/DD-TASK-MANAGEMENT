...
import { api } from '../api'
import { supabase, nextTaskId } from '../supabaseClient'
...
// ─── Employee ID <-> name helpers ──────────────────────────────────────────
// assigned_by / assigned_to now store EMPLOYEE IDs (names can be duplicated),
// so read-time we translate IDs back to names for display.
const EMP_ID_RE = /^EMP-\d+$/i

const buildEmpMaps = (employees) => {
  const idToName = {}
  const idToEmail = {}
  const nameToId = {}
  ;(employees || []).forEach((e) => {
    if (e.id) {
      ...
      idToEmail[String(e.id)] = e.email
    }
    if (e.name) nameToId[String(e.name).toLowerCase()] = e.id
  })
  return { idToName, idToEmail, nameToId }
}
...
const isEmpId = (val) => EMP_ID_RE.test(String(val || '').trim())

const mapWebhookTaskToApp = (item, employees = []) => {
  if (!item) return null
  ...
  const validDepartments = ['SEO', 'SOCIAL MEDIA', 'WEBSITE', 'GRAPHIC', 'HR', 'ACCOUNT', 'AMC', 'SALES', 'APPLICATION', 'COMMON']
  ...
  const { idToName } = buildEmpMaps(employees)
  const rawAssignedTo = data["Assigned To"] || data.assignedTo || ''
  const rawAssignedBy = data["Assigned By"] || data.assignedBy || ''
  const rawEmpIds = data["Employee IDs"] || data.employeeIds || data.employeeId || ''
  const rawAssignedEmails = data["Assigned Emails"] || data.assignedEmails || data.assignedEmail || ''
  const displayAssignedTo = resolveAssigneeValue(rawAssignedTo, idToName)
  const displayAssignedBy = isEmpId(rawAssignedBy) ? (idToName[String(rawAssignedBy).trim()] || rawAssignedBy) : (rawAssignedBy || 'User')
  ...
  return {
    ...
    employeeId: rawEmpIds,
    employeeIds: rawEmpIds,
    assignedEmail: rawAssignedEmails,
    assignedEmails: rawAssignedEmails,
...
export function AppProvider({ children }) {
  ...
  useEffect(() => {
    let interval;
    if (activeTimer) {
      setSessionSecs(Math.floor(getActiveTimerMs(activeTimer) / 1000));
  ...
  const [payments, setPayments] = useState(() => {
    try {
      const saved = localStorage.getItem('dd_payments_v1')
      if (saved) return JSON.parse(saved)
    } catch (err) {}
    return []
  })
  ...
  useEffect(() => {
    localStorage.setItem('dd_payments_v1', JSON.stringify(payments))
  }, [payments])
  ...
  const updateTask = async (id, fields) => {
    ...
    } else if ((fields.assignedTo && fields.assignedTo !== currentTask.assignedTo) || (fields.assignedToIds && fields.assignedToIds !== currentTask.assignedToIds)) {
    ...
    try {
      const data = await api.post({
        action: 'update_task',
          ...
          assignedBy: fields.assignedById !== undefined ? fields.assignedById : (mergedTask.assignedById || mergedTask.assignedBy),
          assignedTo: fields.assignedToIds !== undefined ? fields.assignedToIds : (mergedTask.assignedToIds || mergedTask.assignedTo),
          employeeIds: mergedTask.employeeIds || '',
          assignedEmails: fields.assignedEmails !== undefined ? fields.assignedEmails : (mergedTask.assignedEmails || mergedTask.assignedEmail || ''),
    ...
    try {
      const res = await api.post({
        ...
        taskId: newTask.id,
          ...
          assignedBy: newTask.assignedById || newTask.assignedBy,
          assignedTo: newTask.assignedToIds || newTask.assignedTo,
          employeeIds: newTask.employeeIds || newTask.employeeId || '',
          assignedEmails: newTask.assignedEmails || newTask.assignedEmail || '',
        ...
        if (initialTaskIds.current === null) {
          ...
          newTasksList.forEach(nt => {
            ...
            if (!hasId) {
              ...
              if (nt.isAutoGenerated && !nt.dueDate && nt.recurringSchedule) {
                const nextDue = computeRecurringDueDate(nt.recurringSchedule, nt.recurringDay, nt.recurringMonths, new Date(), true)
                if (nextDue) {
                  updateTask(nt.id, { dueDate: nextDue })
                }
              }
            } else {
    ...
    try {
      ...
      items.forEach(item => {
        ...
        if (item.taskId || item.id || item["Task ID"]) {
          const mapped = mapWebhookTaskToApp(item, employees)
          if (mapped) {
  ...
  // Re-fetch tasks once the team list arrives, so any stored EMP-xxx IDs that
  // couldn't be translated yet (tasks fetched before employees) render as names.
  const hasUnresolvedEmpIds = useMemo(
    () => employees.length > 0 && tasks.some(t => /\bEMP-\d+\b/i.test(String(t.assignedTo || '') + ' ' + String(t.assignedBy || ''))),
    [employees, tasks]
  )
  useEffect(() => {
    if (hasUnresolvedEmpIds) fetchSyncedTasks()
  }, [hasUnresolvedEmpIds]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  }, [profile, deletedPersonalChatIds, isPunchedIn])
  ...
  useEffect(() => {
    ...
    const paymentInterval = setInterval(fetchPayments, 300000) // 5 minutes
