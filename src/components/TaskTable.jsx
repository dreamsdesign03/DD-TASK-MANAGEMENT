                                    ...
                                    >
                                      <div className="p-4 space-y-3">
                                        <div className="flex justify-between items-start">
                                         ...
                                         <h3 className="text-sm flex items-center gap-2 flex-wrap">
                                           <span className={task.isRecurring ? "font-extrabold italic text-[#702c91]" : "font-semibold text-[#702c91]"}>
                                             {task.title}
                                           </span>
                                           {task.isRecurring && (
                                             <span className="bg-purple-100 border border-purple-300 text-[#702c91] text-[10px] font-extrabold px-2 py-0.5 rounded-full inline-flex items-center gap-1 shadow-2xs">
                                               <span className="material-symbols-outlined text-[12px] text-[#702c91]">update</span>
                                               {task.recurringSchedule ? `Recurring (${task.recurringSchedule})` : 'Recurring'}
                                             </span>
                                           )}
                                         </h3>
                                    ...
                                    <div className="flex flex-col items-start gap-1 w-full md:w-auto">
                                      <div className="flex items-center gap-2 flex-wrap">
                                         <a
                                           ...
                                           className={`text-[14px] cursor-pointer transition-all hover:underline ${
                                             task.isRecurring ? 'font-extrabold italic text-[#702c91] tracking-wide' : 'font-bold text-[#702c91]'
                                           }`}
                                         >
                                           {task.title}
                                         </a>
                                         {task.isRecurring && (
                                           <span
                                             className="bg-purple-100 border border-purple-300 text-[#702c91] text-[10px] font-extrabold px-2 py-0.5 rounded-full inline-flex items-center gap-1 shadow-2xs shrink-0"
                                             title={`Recurring Task (${task.recurringSchedule || 'Recurring'})`}
                                           >
                                             <span className="material-symbols-outlined text-[13px] text-[#702c91]">update</span>
                                             {task.recurringSchedule ? `Recurring (${task.recurringSchedule})` : 'Recurring'}
                                           </span>
                                         )}
                                        {!task.isRecurring && (
                              ...
                              return (
                                ...
                                >
                                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                                   ...
                                   <h3 style={{ fontSize: 15, fontWeight: 700, fontStyle: task.isRecurring ? 'italic' : 'normal', color: task.isRecurring ? '#702c91' : '#1E1B2E', margin: '0 0 8px 0', lineHeight: 1.2, transition: 'color 0.2s', display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}
                                     onMouseEnter={(e) => { e.currentTarget.style.color = '#702c91'; e.currentTarget.style.textDecoration = 'underline'; }}
                                     onMouseLeave={(e) => { e.currentTarget.style.color = task.isRecurring ? '#702c91' : '#1E1B2E'; e.currentTarget.style.textDecoration = 'none'; }}
                                   >
                                     {task.title}
                                     {task.isRecurring && (
                                       <span className="bg-purple-100 border border-purple-300 text-[#702c91] text-[10px] font-extrabold px-2 py-0.5 rounded-full inline-flex items-center gap-1 shadow-2xs">
